...
在 `content/exhibit-scenes.ts` 修改章节顺序、镜头、图层、主媒体、讲解、点位和来源。自动导览唯一读取 `tourChapters[].durationSeconds`，五章总时长必须保持 300 秒；不要在组件中创建第二套计时数据。章节切换由上层统一控制，不要在展框组件中复制数据或创建第二张地图。
