...
- 三项证据分别保留医学内容、公开授权和现场证据范围边界，已有照片与视频按事件归组展示。
...
- 连续导览已进一步派生自动分镜：地图位点镜头、精选图片、关联现场视频、点位摘要与来源文字会在章内自动切换；用户主动播放后可听现场原声，并可在统一播放栏持续静音或恢复声音。
- P12 已恢复现场视频的可选 AAC 音轨；无原音轨的输入仍可处理。`live-melon.mp4` 属于学校可公开的宣传作品，在保留既有隐私画面裁剪的同时恢复其原声。
- P13 已加入全站单实例背景音乐：`Forest Mist Whispers → Valley Sunset → Rest Now` 固定循环，发布副本本地托管于 `public/audio/` 并压为约 128 kbps；用户可在主导航和导览播放栏开关 BGM，曲目、作者与 Mixkit Free License 在页脚可追溯。
- P13 的 BGM 默认音量约 14%；任一站内视频播放时自动在约 0.7 秒内 duck 到不高于 2%，视频暂停/结束后约 1.2 秒恢复，用户主动关闭 BGM 的选择优先且不会被自动导览重新打开。
- 自动导览的视频分镜改由媒体 `currentTime` 驱动进度，移动端缓冲期间不再继续走墙钟；暂停/继续同步控制实际视频。标称 240.783 秒时间轴及所有分镜时长保持不变。
...
- 构建命令：`npm run build`
- 测试命令：`npm test`
- 当前结构测试：24 项（新增全局 BGM 本地资产、固定循环、视频 ducking 与授权追溯保护；其余仍覆盖导览时长、overview/detail 镜头、marker label、drawer pointer、focus PNG-alpha/provenance、增强独立失败、单 MapLibre、无在线运行时底图与无 3D terrain）
...
- [x] P12：恢复现场视频原声与自动导览声音控制，包括已获公开授权的直播助农宣传素材
- [x] P13：加入三首本地托管背景音乐的全站固定循环、全局开关与视频优先自动 ducking
