...
网站以五章故事和单一互动地图呈现实践足迹、绿洲水脉、药材产业和人物故事。首页可一键开始连续导览，也可进入自由地图；导览会自动轮播每章主图、每个点位的一张不重复关联图、全部静音视频与来源摘要。章节依据、自动分镜和来源反向用途由 `content/evidence-index.ts` 从现有内容关系自动派生。
...
自动导览时长由 `chapterFramesById` 派生：纯文字 4 秒、图片 12 秒、视频按文件真实时长播放一次。媒体在五章内不重复，章节点位会优先选取尚未播放的关联图片。新增来源必须填写类型、主题和支撑摘要，并至少被一个章节、点位、水脉阶段或资源条目实际引用。
