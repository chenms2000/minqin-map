...
import { herbs, media, sourceById, sources, storyPoints, waterTimeline } from "@/content";
...
export function LongFormPage({ mapSlot, showAllMedia, onToggleMedia, onStartExhibit, onEnterMap }: LongFormPageProps) {
  ...
  return <>
    ...
    <section className="dossier" id="dossier" aria-labelledby="dossier-title">
      ...
      <div className="dossier-card"><dl><div><dt>项目</dt><dd>民勤生态治理背景下中医药资源利用与“一带一路”绿色传播实践</dd></div><div><dt>团队</dt><dd>北京中医药大学生命科学学院<br />绿洲药韵·丝路智传实践团</dd></div><div><dt>实践时间</dt><dd>2026年8月3—4日（首期素材）</dd></div><div><dt>方法</dt><dd>实地观察、劳动记录、影像采集、公开资料核验与数字地图叙事</dd></div></dl><div className="dossier-stats"><article><strong>{visitedPoints}</strong><span>团队实践点</span></article><article><strong>{publicPoints}</strong><span>公开知识点</span></article><article><strong>{featuredMediaCount}</strong><span>导览精选影像</span></article><article><strong>{sources.length}</strong><span>公开资料来源</span></article></div><p className="boundary-note"><b>内容边界</b> 团队到访与公开知识点分开统计；无GPS位置只到县域或村级；未鉴定植物不擅自命名；医学内容只作资源、生态与产业介绍。</p></div>
    </section>
    ...
    <section className="field-media" id="field-media" aria-labelledby="media-title">
      <div className="section-index">03 / 实践影像</div><div className="media-heading"><div><p className="eyebrow">TWO DAYS IN THE FIELD</p><h2 id="media-title">两天，不止两组照片。</h2></div><p>精选影像先呈现完整叙事，视频均由用户点击后播放，不自动播放。展开后可查看首期全部20项素材。</p></div>
      <div className="media-days">{mediaDays.map((day) => { const assets = media.filter((asset) => asset.capturedAt.startsWith(day.key) && (showAllMedia || asset.featured)); return <article key={day.key} className="media-day"><div className="media-day-title"><strong>{day.label}</strong><span>{day.title}</span></div><div className="media-grid">{assets.map((asset) => <figure key={asset.id}>{asset.type === "image" ? <img src={asset.src} alt={asset.alt} loading="lazy" /> : <video src={asset.src} poster={asset.poster} controls preload="none" aria-label={asset.alt}><track kind="captions" src="/media/shared/ambient-zh.vtt" srcLang="zh" label="中文说明" default /></video>}<figcaption><time>{asset.capturedAt.slice(11)}</time><p>{asset.caption}</p>{asset.featured && <b>导览精选</b>}</figcaption></figure>)}</div></article>; })}</div>
    ...
    <footer id="sources"><div className="footer-brand"><span className="brand-mark">绿</span><div><strong>民勤中医药生态文化数字地图</strong><small>首期正式成果 · 绿洲药韵·丝路智传实践团</small></div></div><div className="footer-sources"><h2>资料来源</h2>{sources.map((source, index) => <a key={source.id} href={source.url} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div>{source.title}<small>{source.publisher} · {source.publishedAt}</small></div><b>↗</b></a>)}</div><div className="method-note"><strong>资料与方法说明</strong><p>团队照片与视频摄于2026年8月3—4日；公开资料用于历史、生态、科研、人物与产业背景。公开知识点不计入团队到访，近似坐标不作导航用途。网站底图为本地 PMTiles，底图失败时文字与影像仍可浏览。</p></div><div className="footer-note"><p>隐私与医学边界：不展示手机号、学号、邮箱；不使用未经证实采访引语；不作药物疗效宣传。</p><p>版本：首期正式成果 · 更新于 2026.08.07</p></div></footer>
  </>;
}
