...
type LongFormPageProps = {
  ...
  onToggleMedia: () => void;
  onStartExhibit: (initialIndex?: number) => void;
  storyIndex: number;
...
export function LongFormPage({ mapSlot, showAllMedia, onToggleMedia, onStartExhibit, storyIndex, storyTargetId, onStoryChapterChange, onStoryMapModeChange, onStoryTargetHandled }: LongFormPageProps) {
  ...
  return <>
    ...
    <section className="field-media" id="field-media" aria-labelledby="media-title">
      <div className="section-index">A3 / 完整实践影像</div><div className="media-heading"><div><p className="eyebrow">TWO DAYS IN THE FIELD</p><h2 id="media-title">两天，五类行动。</h2></div><p>采摘观察、主题科普、直播助农、浇水维护与基地记录共同构成两日实践。视频均由用户点击后播放；展开后可查看首期全部 {media.length} 项素材。</p></div>
