...
test("continuous tour preserves optional现场 sound and rotates derived frames", async () => {
  ...
  assert.match(experience, /activeTourFrameMedia\?\.type === "video"/);
  assert.match(experience, /activeTourFrame\?\.id, activeTourFrameMedia\?\.type/);
  assert.match(experience, /activeTourFrameStartSeconds \+ Math\.max\(0, currentTimeSeconds\)/);
  assert.match(experience, /onTourVideoTimeUpdate=\{syncTourVideoProgress\}/);
  assert.match(experience, /onTourVideoEnded=\{completeTourVideo\}/);
  ...
  assert.match(stage, /useRef<HTMLVideoElement>\(null\)/);
  assert.match(stage, /if \(isPlaying\) void video\.play\(\)/);
  assert.match(stage, /else video\.pause\(\)/);
  assert.match(stage, /onVideoTimeUpdate\(event\.currentTarget\.currentTime\)/);
  assert.match(stage, /onEnded=\{onVideoEnded\}/);
